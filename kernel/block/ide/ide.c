...
u8 ideInit(diskID_t id)
{
  ...
  return 0; /* Pour �iter les warnings mais ne devrait pas �re ex�ut�*/
}
...
u8 ideGetSize(diskID_t id, u32 *size)
{
  ...
  return 0; /* Pr�ent pour �iter les warnings, ne devrait pas �re ex�ut�*/
}
...
u8 idepRead(idePhysicalDiskID_t id, u32 cnt, lbaAddress_t log, u8 *buffer)
{
  ...
  if(d.type == ideCd)
	...
	return BLOCK_ERR_MEMORY;
      ...
      if(error == BLOCK_ERR_OK)
	{
	  /* Copier les secteurs demands dans buffer */
	  for(c=0;c<512 * cnt;c++)
...
s16 idepSendCommand(idePhysicalDiskID_t id, ideParam_t param)
{
  ...
  /* Attendre 400ns avant de lire l'�at du contrleur */
